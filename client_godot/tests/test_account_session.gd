...
func run() -> void:
	...
	var session = Session.new(Api.new(security), store, folder + "/account.cfg")
	...
	session.queue_free()
	await process_frame
	var restored = Session.new(Api.new(security), Store.new(security, folder + "/tokens"), folder + "/account.cfg")
	...
	await restored.perform("login", server, {"username":"test", "password":"synthetic-password"}, true)
	store.save(server, "test", "invalid")
	restored.queue_free()
	await process_frame
	var rejected = Session.new(Api.new(security), store, folder + "/account.cfg")
